...
main (int argc, char** argv)
{
  ...
  {
#if defined(_WIN32)
    fprintf(errlog, "Usage: visualinfo [-a] [-s] [-h] [-pf <id>]\n");
    fprintf(errlog, "        -a: show all visuals\n");
    fprintf(errlog, "        -s: display to log instead of visualinfo.txt\n");
    fprintf(errlog, "        -pf <id>: use given pixelformat\n");
    fprintf(errlog, "        -h: this screen\n");
#else
    fprintf(errlog, "Usage: visualinfo [-h] [-display <display>] [-visual <id>]\n");
    fprintf(errlog, "        -h: this screen\n");
    fprintf(errlog, "        -display <display>: use given display\n");
    fprintf(errlog, "        -visual <id>: use given visual\n");
#endif
    return 1;
  }
  ...
  {
    fprintf(errlog, "Error: CreateContext failed\n");
    DestroyContext(&ctx);
    return 1;
  }
...
#  endif
  ...
  if (GLEW_OK != err)
  {
    fprintf(errlog, "Error [main]: glewInit failed: %s\n", glewGetErrorString(err));
    DestroyContext(&ctx);
    return 1;
  }
